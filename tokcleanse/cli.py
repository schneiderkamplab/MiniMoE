...
def train_command(
    ...
    router_learning_rate_multiplier: float = typer.Option(
        1.0,
        "--router-learning-rate-multiplier",
        help="Learning-rate multiplier for router parameters only.",
    ),
    ...
    lr_warmup_steps: int = typer.Option(
        DEFAULT_TRAIN_LR_WARMUP_STEPS,
        "--lr-warmup-steps",
        help="Number of optimizer steps for linear warmup before cosine decay to 1e-8 over the resolved total steps.",
    ),
    device: str = typer.Option(
        "auto",
        "--device",
    ...
    return json.dumps(
        {
            "dry_run": {
                ...
                "router_learning_rate_multiplier": report.router_learning_rate_multiplier,
