...
class Gemma4TextRouter(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if expert1_logit_bias is not None and expert_scores.shape[-1] > 1:
            ...
            expert_scores = expert_scores.clone()
            expert_scores[:, 1] = expert_scores[:, 1] + bias
...
class OdinMOETextDecoderLayer(GradientCheckpointingLayer):
    """Gemma 4 text decoder layer with a pure routed-expert feed-forward block."""
...
class OdinMOETextModel(Gemma4TextModel):
    """Gemma 4 text model that uses OdinMOE decoder layers."""

    config: OdinMoETextConfig

    _can_record_outputs = {
        "router_logits": OutputRecorder(Gemma4TextRouter, index=0),
        "hidden_states": OdinMOETextDecoderLayer,
        "attentions": Gemma4TextAttention,
    }
    ...
    def __init__(self, config: Gemma4TextConfig):
        ...
        self.layers = nn.ModuleList(
            [OdinMOETextDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = Gemma4RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = Gemma4TextRotaryEmbedding(config)
        self.gradient_checkpointing = False
...
@auto_docstring(custom_intro="The OdinMOE language model with a language modeling head.")
class OdinMOEForCausalLM(Gemma4ForCausalLM):
    """Gemma 4 causal LM variant with pure routed-expert decoder layers."""

    config_class = OdinMoETextConfig
    config: OdinMoETextConfig

    def __init__(self, config: OdinMoETextConfig):
        Gemma4PreTrainedModel.__init__(self, config)
        self.model = OdinMOETextModel(config)
        self.vocab_size = config.vocab_size
        self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
        self._keys_to_ignore_on_load_unexpected = [
            f"model.{name}" for name in self.model._keys_to_ignore_on_load_unexpected
